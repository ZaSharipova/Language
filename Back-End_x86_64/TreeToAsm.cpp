...
static void PrintFunction(LangNode_t *func_node, AsmCtx *ctx);
...
static void EmitEpilogue(AsmCtx *ctx) {
    ...
    if (ctx->sub_info->frame_size > 0) {
        EMIT("add rsp, %d", ctx->sub_info->frame_size);
    }
...
static void PrintFunction(LangNode_t *func_node, AsmCtx *ctx) {
    ...
    int param_count = ctx->arr->var_array[func_node->left->value.pos].variable_value;

    int slot_counter = 0;
    if (args) {
        AssignParamSlots(args, ctx->arr, &slot_counter);
    }
    ...
    ctx->asm_info->counter = slot_counter;
    CountLocalSlots(body, ctx->arr, ctx->asm_info);

    int total_slots = ctx->asm_info->counter;
    ...
    fprintf(ctx->file, ";---------------------------------------------\n");
    fprintf(ctx->file, "; function: %s  (params=%d, locals=%d, frame=%d)\n", func_name, param_count, local_slots, frame_size);
    fprintf(ctx->file, ";---------------------------------------------\n");
    if (is_main) {
        EMIT_LABEL("main:");
    }
...
#pragma GCC diagnostic ignored "-Wswitch-enum"
    switch (expr->value.operation) {
        ...
        case kOperationAdd: EmitBinaryOp(expr, ctx, "add", "addition"); break;
        case kOperationSub: EmitBinaryOp(expr, ctx, "sub", "subtraction"); break;
        case kOperationMul: EmitBinaryOp(expr, ctx, "imul", "multiplication"); break;
        case kOperationDiv: EmitBinaryOp(expr, ctx, "idiv", "division"); break;
...
static void PrintStatement(LangNode_t *stmt, AsmCtx *ctx) {
    ...
    switch (stmt->type) {
        case kOperation: PrintStatementOperationCase(stmt, ctx); break;
        case kVariable: PopToVar(stmt, ctx); break;
...
static void PrintArrDeclare(LangNode_t *stmt, AsmCtx *ctx) {
    ...
    int base_slot = ctx->arr->var_array[stmt->left->left->left->value.pos].pos_in_code;
...
static void PrintIsForArray(LangNode_t *stmt, AsmCtx *ctx) {
    ...
    /* stack: [value, index] */
    PrintExpr(stmt->right, ctx);                 // push value
    PrintExpr(stmt->left->right, ctx);           // push index
...
static void EmitDraw(LangNode_t *node, AsmCtx *ctx) {
    assert(node);
    assert(ctx);
...
#pragma GCC diagnostic ignored "-Wswitch-enum"
    switch (stmt->value.operation) {
        ...
        case kOperationReturn: PrintReturn(stmt, ctx); break;
        case kOperationWrite: EmitPrintInt(stmt, ctx); break;
        ...
        case kOperationDraw: EmitDraw(stmt, ctx); break;
